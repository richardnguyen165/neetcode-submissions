    ...
    def validPalindrome(self, s: str) -> bool:
        ...
        while left < right:
            if s[left] == s[right]:
                ...
                # Simple check if we skip either one, check if the other option is available
                # We are only allowed one mistake, so this is the final decision

                '''
                Error: Only trying one deletion option, you must try both
                '''
                if left_option:
                    return True
                elif right_option: 
